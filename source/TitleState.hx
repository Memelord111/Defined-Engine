...
class TitleState extends MusicBeatState
{
	...
	var gradientBar:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width, 1, 0xFF0657F7);
	...
	function startIntro()
	{
		...
		switch(easterEgg.toUpperCase())
		{
			...
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
				gfDance.frames = Paths.getSparrowAtlas('D&B Title');
		...
		logo = new FlxSprite(-1100, -25).loadGraphic(Paths.image('logo'));
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('DE-Logo'));
		add(ngSpr);
		ngSpr.visible = false;
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 6:
					...
					#else
					createCoolText(['Unassociated', 'with'], -40);
					#end
				case 8:
					addMoreText('Definitive Edition', -40);
					ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 9:
					deleteCoolText();
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			FlxTween.tween(logo,{x: 15}, 1.4, {ease: FlxEase.expoInOut});

			FlxTween.angle(logo, -7, 7, Conductor.crochet / 300, {ease: FlxEase.quartInOut, type: PINGPONG});
